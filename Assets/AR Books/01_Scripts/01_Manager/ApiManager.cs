...
//using Ecommerce;Akash
//using Ecommerce.checkout;Akash
...
public struct Products
{
    public bool success;
    public int code;
    //public List<Product> data; Akash
    public string message;
}
...
public class ApiManager : MonoBehaviour
{
    ...
    public void APIResponseFailPopup(long statusCode, string errorTitle, string responseData)
    {
        if (!IsValid(statusCode))
        {
            ...
            errorWindow.SetErrorMessage("Something Went Wrong", Validator.GetValidateErrorMessage(ErrorMessage), "CLOSE", ErrorWindow.ResponseData.JustClose);
        }
        else
        {
            ...
            errorWindow.SetErrorMessage(errorTitle, Validator.GetValidateErrorMessage(ErrorMessage), "CLOSE", ErrorWindow.ResponseData.JustClose);
        }
    }
    ...
    IEnumerator CheckInternetConnection(Action<bool> action)
    {
        ...
        {
            errorWindow.SetErrorMessage("Internet Connection Failed !", "Please, Check you internet connection and try again.", "TRY AGAIN",ErrorWindow.ResponseData.InternetIssue);
            RaycastUnblock();
            action(false);
        }
        else
    ...
    private  void RegisterUser(bool success, object data, long statusCode) {
        if (success)
        {
            ...
            APIResponseFailPopup(statusCode, "Something Went Wrong", data.ToString());
        }
        RaycastUnblock();
    }
        ...
        if (success)
        {
            ...
            else
            {
                ...
                switch (error.code)
                {
                    ...
                    case 401:
                        errorWindow.SetErrorMessage("Server Problem !", Validator.GetValidateErrorMessage(ErrorMessage), "TRY AGAIN", ErrorWindow.ResponseData.InternetIssue);
                        break;
                    case 403:
                        errorWindow.SetErrorMessage("Session Expire", Validator.GetValidateErrorMessage(ErrorMessage), "RE-LOGIN", ErrorWindow.ResponseData.SessionExpire);
                        break;
                    default:
                        errorWindow.SetErrorMessage("Error Found !", Validator.GetValidateErrorMessage(ErrorMessage), "CLOSE", ErrorWindow.ResponseData.JustClose);
                        break;
                }
            }
        }
    ...
    private void OrderUpdated(bool success, object data, long statusCode)
    {
        ...
        {
            APIResponseFailPopup(statusCode, "Error Found !", data.ToString());
        }
        RaycastUnblock();
    }
